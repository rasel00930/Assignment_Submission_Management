...
export default function SubjectsPage() {
  ...
  return <RolePage role="Admin"><PageHeader title="Subjects" description="Maintain subject names and unique subject codes." actions={<Button onClick={() => edit()}><Plus className="h-4 w-4" /> Add subject</Button>} /><div className="mb-4 flex items-center gap-2"><input id="inactiveSubjects" type="checkbox" checked={includeInactive} onChange={(e) => setIncludeInactive(e.target.checked)} className="h-4 w-4" /><label htmlFor="inactiveSubjects" className="text-sm font-semibold text-slate-600">Show inactive</label></div>
