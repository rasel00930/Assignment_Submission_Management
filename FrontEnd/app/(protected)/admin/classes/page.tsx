...
export default function ClassesPage() {
  ...
  return <RolePage role="Admin"><PageHeader title="Classes & courses" description="Create academic groups, sections and academic years." actions={<Button onClick={() => edit()}><Plus className="h-4 w-4" /> Add class</Button>} />
    <div className="mb-4 flex items-center gap-2"><input id="inactiveClasses" type="checkbox" checked={includeInactive} onChange={(e) => setIncludeInactive(e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-indigo-600" /><label htmlFor="inactiveClasses" className="text-sm font-semibold text-slate-600">Show inactive</label></div>
